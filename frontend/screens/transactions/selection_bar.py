...
def build_selection_bar(selected_ids: Set[str],
    ...
    return ft.Container(
        content=ft.Row([
            ft.Text(tr("transactions.selected").replace("{count}", str(count)), size=scaled(10), color=t.ACCENT,
                    font_family="monospace"),
            ft.Row([
                ft.TextButton(tr("transactions.delete"),
                              style=ft.ButtonStyle(color=t.RED),
                              on_click=on_delete),
                ft.Text("·", size=scaled(10), color=t.TEXT_DIMMER),
        ...
        padding=t.pad_sym(horizontal=scaled(18), vertical=scaled(5)),
