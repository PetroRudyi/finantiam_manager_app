...
import flet as ft
from backend.config import GEMINI_MODELS, DEFAULT_GEMINI_MODEL
...
def build_api_key_editor(settings, on_save, on_cancel) -> ft.Column:
    ...
    Parameters:
        settings: AppSettings with current gemini_api_key and gemini_model
        on_save: callback(new_key: str, new_model: str) when user saves
    ...
    model_dd = ft.Dropdown(
        value=current_model,
        label=tr("api_key_editor.model"),
        bgcolor=t.SURFACE2,
        border_color=t.BORDER,
        focused_border_color=t.ACCENT,
        border_radius=scaled(DD_RADIUS),
        label_style=ft.TextStyle(size=scaled(FONT_SM), color=t.TEXT_DIMMER, font_family="monospace"),
    ...
    def _save(e):
        on_save(api_key_field.value.strip(), model_dd.value or DEFAULT_GEMINI_MODEL)
